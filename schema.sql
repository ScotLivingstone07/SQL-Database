...
--
-- Host: localhost
-- Generation Time: Apr 15, 2018 at 01:29 PM
...
CREATE TABLE IF NOT EXISTS `FileItem` (
  ...
  `Title` varchar(255) CHARACTER SET cp1251 DEFAULT '""' COMMENT 'Title of the item stored',
  `Location` varchar(255) CHARACTER SET cp1251 DEFAULT '""' COMMENT 'Location of the item (where stored)',
...
ALTER TABLE `Assignment`
  ADD KEY `Project.ProjectID` (`ProjectID`),
  ADD KEY `Employee.EmployeeID` (`EmployeeID`),
...
ALTER TABLE `EquipmentLoan`
  ADD KEY `Employee.EmployeeID` (`EmployeeID`),
  ADD KEY `Equipment.EqupimentID` (`EquipmentID`),
  ADD KEY `EmployeeID` (`EmployeeID`,`EquipmentID`);
...
--
ALTER TABLE `Assignment`
  ADD CONSTRAINT `Assignment_ibfk_2` FOREIGN KEY (`ProjectID`) REFERENCES `Project` (`ProjectID`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `Assignment_ibfk_1` FOREIGN KEY (`EmployeeID`) REFERENCES `Employee` (`EmployeeID`) ON DELETE CASCADE ON UPDATE CASCADE;
